function select_lang(lng)
{
  {% for item in site.data.lang %}
    if (lng == "{{ item.id }}")
    {
      ...
      $("#lang_menu_{{ item.id }}").removeClass('menu_item');
      $("#lang_menu_{{ item.id }}").addClass('menu_item_active');
    }
    else
    {
      var display = 'none';
      $("#lang_menu_{{ item.id }}").removeClass('menu_item_active');
      $("#lang_menu_{{ item.id }}").addClass('menu_item');
    }
